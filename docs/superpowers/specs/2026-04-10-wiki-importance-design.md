...
- `counts`: 동작별 원시 카운트 (6종)
- `importance`: 직접 관심 집계값 (counts에서 가중치 적용 자동 계산)
- `hit_count`: 검색 노출 집계값 (counts에서 자동 계산)
- `last_accessed`: 마지막 접근일 (show/update/검색히트 시 갱신)

### 하위 호환

- 기존 wiki 페이지에 `counts`가 없으면 모두 0으로 간주
- 마이그레이션 불필요 — 첫 접근 시 자동으로 counts 생성

## 2. 카운트 증가 조건

| 동작 | counts 필드 | importance 가중치 | hit_count 가중치 |
|------|------------|-------------------|------------------|
| `wiki show` | show +1 | ×1 | — |
| `wiki update --append` | append +1 | ×2 | — |
| `wiki update --body` | update +1 | ×1 | — |
| `wiki link` | link +1 (양쪽) | ×1 | — |
| `wiki create` → 유사 병합 | merge +1 | ×3 | — |
| `search` 결과에 wiki 히트 | search_hit +1 | — | ×1 |
| `query` 결과에 wiki 히트 | query_hit +1 | — | ×1 |

### 계산 공식

```
importance = show×1 + append×2 + update×1 + link×1 + merge×3
hit_count  = search_hit + query_hit
```

### 카운트 증가 방지

- `--no-count` 플래그: show/update 시 카운트 증가 안 함 (스크립트/자동화용)
- `wiki list`, `wiki index`, `wiki rm` → 카운트 변동 없음

## 3. reset-importance

```sh
# importance 계열만 초기화 (show, append, update, link, merge counts + importance 값)
hwicortex wiki reset-importance --project demo

# 모든 카운트 전부 초기화 (importance + hit_count + 모든 counts)
...
1. 기존 페이지 body에 구분선 + 새 내용 append:
   ...
   만료 시 갱신 로직...
   ```
2. `counts.merge` +1, importance 재계산
...
1. 새 페이지 생성 (importance 초기값: counts 모두 0)
...
```dataview
TABLE counts.show as "조회", counts.append as "보강",
      counts.merge as "병합", importance as "관심도",
      counts.search_hit as "검색", counts.query_hit as "쿼리",
      hit_count as "노출"
FROM "wiki/{project}"
...
| 둘 다 높음 | 핵심 지식 |
| merge 높음 | 여러 주제가 수렴하는 허브 페이지 |

## 6. 코드 변경 범위

| 파일 | 변경 내용 |
|------|-----------|
| `src/wiki.ts` | `WikiMeta`에 counts/importance/hit_count/last_accessed 추가. `bumpCount()`, `recalcImportance()`, `resetImportance()`, `findSimilar()` 함수 추가 |
| `src/cli/wiki.ts` | `show`에 카운트 증가, `create`에 유사도 체크+프롬프트, `reset-importance` 서브커맨드, `--no-count`/`--auto-merge`/`--force` 플래그 |
| `src/store.ts` (또는 검색 결과 반환 지점) | search/query 결과에 wiki 페이지 포함 시 hit_count 증가 |
| `test/wiki.test.ts` | 카운트 증가/리셋, 유사도 감지, 병합, 가중치 계산 테스트 |
